...
class MarketingForm(forms.ModelForm):
    image = forms.FileField(label='Выберите фото')
    attributes = forms.CharField(widget=forms.Textarea(
        attrs={'class': 'textarea', 'placeholder': 'Описание'}
    ), label='Описание')
    finished = forms.CharField(label='Длительность', widget=forms.TextInput(attrs={'class': 'input'}))
    slug = forms.CharField(widget=forms.TextInput(
        attrs={'class': 'input', 'placeholder': 'Slug'}
    ), label='Ссылка')
